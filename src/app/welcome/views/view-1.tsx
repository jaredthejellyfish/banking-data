...
import { login, signup } from './action/login-signup';
  ...
  return (
    <>
      <div className="md:max-w-[50%] flex flex-col gap-y-4 w-full">
        ...
        <p className="text-lg">
          To use App Name, you must have an account so lets get started by
          logging in or creating an account
        </p>
        <form action={incrementProgress}>
          <div className="flex flex-col gap-y-3 mb-4 sm:max-w-[60%]">
            <input
              type="email"
              name="email"
              placeholder="Email"
              className="bg-black p-2.5 rounded-xl border"
              required
            />
            <input
              type="password"
          ...
          <div className="flex flex-row gap-x-3 group">
            <button
              type="submit"
              formAction={login}
              className="w-[130px] border flex flex-row group-hover:items-center justify-center pr-3 pl-1.5 py-2 rounded-md border-white disabled:opacity-70 bg-white group-hover:bg-transparent group-hover:text-white hover:bg-transparent group text-black hover:text-white transition-all gap-x-2 duration-300"
            >
              <span>Login</span>
